...
class CampaignsPage(MainPage):
    ...
    def create_campaign(self, file_path):
        ...
        for i in range(3):
            ...
            self.enter(self.locators.slide_link(i), url)
            self.enter(self.locators.slide_title(i), f"Заголовок {i + 1}")
        self.upload_file(self.locators.UPLOAD_IMG256_BUTTON, file_path)
        self.click(self.locators.SAVE_IMG_BUTTON)
        self.enter(self.locators.TITLE_FIELD, "Заголовок")
        self.enter(self.locators.TEXT_FIELD, "Текст объявления")
